...
public interface RegisterRequestRepository extends JpaRepository<RegisterRequest, Long> {
  ...
  @Query(value = "SELECT rr FROM RegisterRequest rr JOIN FETCH rr.member m " +
          ...
          "  (:type = 'ORGANIZATION_NAME' AND m.organizationName LIKE %:keyword%) OR " +
          "  (:type = 'REPRESENTATIVE_NAME' AND m.representativeName LIKE %:keyword%) OR " +
          "  ((:type IS NULL OR :type = 'ALL') AND (" +
          "    m.bizNumber LIKE %:keyword% OR " +
          "    m.email LIKE %:keyword% OR " +
          "    m.organizationName LIKE %:keyword% OR " +
          "    m.representativeName LIKE %:keyword%" +
          "  ))" +
          ")",
  ...
  Page<RegisterRequest> findAllWithMember(
          @Param("type") String memberSearchType,
          @Param("keyword") String keyword,
          @Param("status") RegisterStatus registerStatus,
          @Param("role") Role role,
          Pageable pageable);

  @Query("SELECT rr FROM RegisterRequest rr JOIN FETCH rr.member m WHERE m = :member")
  Optional<RegisterRequest> findByMember(@Param("member") Member member);
}
