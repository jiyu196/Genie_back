...
### 📅 260105(월)
* (프론트 요청) -> Query Me 권한 설정 조정 (가입승인 회원 -> 로그인 성공한 회원)
...
### 📅 260111(일)
* S3 Config 및 Util 설정 완료, Attach(공통 첨부파일 서비스 로직 완료), Webtoon 생성 프롬프트 이후 S3 및 db에 저장 성공(프론트, Python 연동)
* 보완사항 우선순위 조정 및 지속 작업 예정(일일 토큰량 제한, 회원가입시 필요 첨부파일 관련, 주문-결제  파트 예외처리, 정기 결제 및 구독 등)
...
* Order 객체 생성 이후 -> 결제창에서 임의로 껐을 때 -> 다시 결제 시도하면 기존 Order 객체 반환 및 결제 로직 구현 (프론트 연동 확인)
* 기존 구독 내역 있으면 -> 결제 차단 로직 구현 (프론트 연동 확인)
* 웹툰 table 내 groupId 컬럼 추가 및 이를 저장하는 로직 추가 -> GroupId 바탕 Webtoon 내 GroupBy 조회 DTO 생성 -> pageable 객체 생성 순차적 백엔드 로직 구현(프론트 확인 필요)

### 📅 260113(화)
* 학습방 내 웹툰 4컷 리스트 페이지 조회 성공(프론트 체크 완료)
* 관리자 매출 조회 백엔드 로직 완료 -> 프론트 단순 리스트 조회 성공, 검색/정렬 기준 프론트 검증 필요

### 📅 260114(수)
* 관리자 페이지 매출 조회 -> 프론트 연동 확인(정렬기준(기간, 결제 상태 등))
* S3 버킷 내 저장된 이미지 다운로드 기능 체크 (프론트 보조)
* 관리자 페이지 내 Member Role 값 변경 로직 추가 (Admin <-> Member), 결제 완료 이후 로직 내 Member Role 값 SUBSCRIBER로 변경 로직 추가

### 📅 260115(목)
* My Page 내 ServiceAccessId 클릭시 개별 ID로 제작한 웹툰 리스트 조회 기능 추가(프론트 연동 확인)
* My Page 웹툰 조회 프론트 단에서 필요한 data 추가로 인한 로직 일부 수정(ServiceAccess pk 전송)
